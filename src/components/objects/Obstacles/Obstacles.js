...
class Obstacles extends Group {
    ...
    update(timeStamp) {
        ...
        this.mesh1.position.set(
            Math.cos(timeStamp * 0.0001 + Math.PI / 2) * 45,
            Math.cos(timeStamp * 0.001) * 2.5,
            Math.sin(timeStamp * 0.0001 + Math.PI / 2) * 45
        );

        // Revolve second obstacle
        this.mesh2.position.set(
            Math.cos(timeStamp * 0.0001 + 0.1) * 55,
            Math.sin(timeStamp * 0.001) * 2.5,
            Math.sin(timeStamp * 0.0001 + 0.1) * 55
        );
    }
}
